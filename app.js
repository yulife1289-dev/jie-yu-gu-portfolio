...
function setNav(page){const nav=page==='project'?'projects':page;document.querySelectorAll('[data-nav]').forEach(a=>a.classList.toggle('active',a.dataset.nav===nav))}
function render(){closeLightbox();if(state.lastPage==='projects')state.projectsScroll=window.scrollY;const r=route();setNav(r.page);if(r.page==='resume')renderResume();else if(r.page==='project')renderProject(r.slug);else renderProjects();scrollTo({top:r.page==='projects'?state.projectsScroll:0,behavior:'instant'});state.lastPage=r.page;requestAnimationFrame(()=>view.focus({preventScroll:true}))}

function renderProjects(){
  document.title='古捷宇｜Interior Design Portfolio';
  const total=state.projects.length,done=state.projects.filter(p=>p.status==='完工作品').length,concept=total-done;
  const range=`01—${String(total).padStart(2,'0')}`;
  view.innerHTML=`<section class="hero"><p class="eyebrow">Interior · Spatial · Visual Design</p><h1>空間的<br><em>敘事者</em></h1><p class="hero-copy">從設計提案到案場執行，將材料、光線與生活尺度編排成可以被感受的空間。精選住宅、商業與工作場域作品。</p></section><section class="section"><header class="section-head"><div><p class="label">Selected Projects · ${range}</p><h2>案例目錄</h2></div><div class="section-meta">${done} Completed Works<br>${concept} Design Proposals</div></header><div class="projects-grid">${state.projects.map((p,i)=>card(p,i)).join('')}</div></section>`;
}
